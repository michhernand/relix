...
TEST_CASE_METHOD(MTCars, "Test LastRelimpAlgorithm.evaluate_column") {
	Model full_model = basic_lm(get_x(), get_y());
	...
	REQUIRE_THAT(
			mean_without,
			Catch::Matchers::WithinRel(0.1774, 0.001)
	);

	auto mean_with = cc.get_mean_with_column();
	REQUIRE_THAT(
			mean_with, 
			Catch::Matchers::WithinRel(0.2172, 0.001)
	);
...
TEST_CASE_METHOD(MTCars, "Test LastRelimpAlgorithm.evaluate_columns") {
	std::unique_ptr<LastRelimpAlgorithm> ra(new LastRelimpAlgorithm);
